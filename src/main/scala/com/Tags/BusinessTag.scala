...
import com.util.RedisUtils.jedisPool
import com.util.{AmapUtil, RedisUtils, Tag, Utils2Type}
...
//      val lat = row.getAs[String]("lat")
  ...
  def getBusiness(long :Double,lat :Double):String = {
    ...
    if(business == null || business.length==0){
      ...
      redis_insertBusiness(geohash,geohash)
    }
    business
  }
  ...
  def redis_queryBusiness(geohash:String):String = {
    val jedisPool = new JedisPool("hadoop01",6379)
